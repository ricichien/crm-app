...
//       this.taskService.updateTask(this.task.id, dtoUpdate).subscribe({
//         next: (res) => {
//           this.loading = false;
//           this.saved.emit(res);
//         },
...
export class TaskFormComponent implements OnChanges {
  ...
  // NEW: when true, hide internal actions & internal close button (dialog wrapper shows actions)
  ...
  // método público usado pelo wrapper (dialog)
  public save() {
    this.serverError = '';
    if (this.form.invalid) {
      this.form.markAllAsTouched();
