...
//   // Declarado aqui; inicializado no construtor (evita usar this.fb antes do construtor)
//   form!: FormGroup;

//   loading = false;
//   error = '';

//   // router é public para uso no template (botão cancelar) — ok deixar público
//   constructor(private fb: FormBuilder, private leadService: LeadService, public router: Router) {
//     // inicializa o form aqui, onde `this.fb` já está disponível
...
//     // getRawValue returns typed values because controls are nonNullable
...
//     // UPDATE path: build Partial payload only with changed properties
...
//     if ((f.company ?? '') !== (this.lead!.company ?? '')) payload.company = f.company || undefined;
...
export class LeadFormComponent implements OnChanges {
  ...
  submit() {
    ...
    // --- UPDATE path (modo edit) ---
    // garantia explícita — se estamos no modo edit, precisamos ter um id válido
    if (this.mode === 'edit') {
      if (
        !this.lead ||
